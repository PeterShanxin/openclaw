...
<<<<<<< HEAD:src/agents/system-prompt.e2e.test.ts
    expect(prompt).toContain("## Current Date & Time");
    expect(prompt).toContain("Time zone: America/Chicago");
    expect(prompt).toContain("Current user time: Monday, January 5th, 2026 — 3:26 PM");
  });
  ...
  it("includes user timezone when provided (24-hour)", () => {
    ...
    expect(prompt).toContain("## Current Date & Time");
    expect(prompt).toContain("Time zone: America/Chicago");
    expect(prompt).toContain("Current user time: Monday, January 5th, 2026 — 15:26");
  });
...
>>>>>>> origin/chore/openclaw-v2026.2.26:src/agents/system-prompt.test.ts
  });
  ...
  it("hints to use session_status for current date/time", () => {
    ...
    expect(prompt).toContain("Do not state an exact current clock time");
  });

  it("includes current user time when provided", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/clawd",
      userTimezone: "America/Chicago",
      userTime: "Monday, January 5th, 2026 — 3:26 PM",
