...
describe("subscribeEmbeddedPiSession", () => {
  ...
  it("does not suppress message_end block replies when message tool sends to a different target", async () => {
    let handler: ((evt: unknown) => void) | undefined;
    const session: StubSession = {
      subscribe: (fn) => {
        handler = fn;
    ...
    const messageText = "This should still be delivered.";

    handler?.({
      type: "tool_execution_start",
      toolName: "message",
      toolCallId: "tool-message-other",
      args: {
        action: "send",
        provider: "telegram",
        to: "318555999",
